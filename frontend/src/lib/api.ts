...
export const gymApi = {
  ...
  updateMember: (memberId: string, body: Record<string, unknown>) =>
    request<MemberResponse>(`/members/${memberId}`, {
      method: "PUT",
      body: JSON.stringify(body),
    }),
  findMemberByPhone: (phoneNumber: string) =>
    request<MemberResponse>(`/members/by-phone?phoneNumber=${encodeURIComponent(phoneNumber)}`),
  ...
  sendInvoiceReceiptPdf: (invoiceId: string, pdfBlob: Blob) => {
    const formData = new FormData();
    formData.append("receipt", pdfBlob, `receipt-${invoiceId}.pdf`);
    
    return fetch(`${API_BASE_URL}/invoices/${invoiceId}/receipt-notifications/pdf`, {
      method: "POST",
      body: formData,
    }).then(async (response) => {
      const contentType = response.headers.get("content-type") ?? "";
