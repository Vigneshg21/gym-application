...
const gymBrand = "HIFI FITNESS";
const receiptPdfMarginMm = 8;
const receiptFileLabel = "Payment Receipt";
...
export default function App() {
  ...
  const receiptActionLockRef = useRef(false);
  ...
  async function handleReceiptDownload(invoice: InvoiceResponse) {
    if (receiptActionLockRef.current) {
      return;
    }

    receiptActionLockRef.current = true;
    setActionBusy(true);
    try {
      const member = members.find((candidate) => candidate.id === invoice.memberId);
      setError(null);
      setNotice(null);
      const pdfBlob = await generateReceiptPdf(invoice, member);
      triggerBrowserDownload(pdfBlob, buildReceiptFileName(invoice));
      setNotice(`${invoice.invoiceNumber} receipt downloaded.`);
    } catch (downloadError) {
      setError(readError(downloadError));
    } finally {
      setActionBusy(false);
      receiptActionLockRef.current = false;
    }
  }

  async function handleSendReceiptPdf(invoice: InvoiceResponse) {
    if (receiptActionLockRef.current) {
      return;
    }
    ...
    try {
      ...
      setNotice(null);
      await runAction(
        "Receipt PDF queued for configured channels.",
        async () => {
          const pdfBlob = await generateReceiptPdf(invoice, member);
          await gymApi.sendInvoiceReceiptPdf(invoice.id, pdfBlob);
        },
      );
    } catch (sendError) {
  ...
  function startEditingMember(member: MemberResponse) {
    ...
    setMemberForm({
      ...
      profileImageBase64: "",
      profileImageContentType: "",
      profileImagePreview: member.profileImageDataUrl ?? "",
    });
  }
    ...
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand-mark">
          <div className="brand-mark__badge">AHF</div>
          <div>
            <p className="brand-mark__eyebrow">HIFI FITNESS</p>
            <h1>Control Room</h1>
          </div>
        </div>
              ...
              <div className="view-stack">
                <div className="two-column-grid">
                  <Panel
                    ...
                    description={editingMemberId ? "Update member details and profile photo." : "Capture core contact details once, then use them across billing, memberships, and campaigns."}
                  >
                    <form
                      className="form-grid"
                      onSubmit={(event) => {
                        event.preventDefault();
                        const successMessage = editingMemberId ? "Member updated successfully." : "Member created successfully.";
                        const operation = editingMemberId
                          ? () =>
                              gymApi.updateMember(
                                editingMemberId,
                                compactPayload({
                        ...
                        void runAction(successMessage, operation, () => stopEditingMember());
                      }}
                    >
                      ...
                      <Field label="Phone number">
                        <input value={memberForm.phoneNumber} onChange={(event) => updateField(setMemberForm, "phoneNumber", event.target.value)} required disabled={editingMemberId ? true : false} />
                      </Field>
                      ...
                      <Field label="Date of birth">
                        <input type="date" value={memberForm.dateOfBirth} onChange={(event) => updateField(setMemberForm, "dateOfBirth", event.target.value)} />
                      </Field>
                      ...
                      <div className="form-actions field--wide">
                        <button type="submit" className="button button--primary" disabled={actionBusy}>{editingMemberId ? "Update" : "Save"} member</button>
                        {editingMemberId ? (
                          <button type="button" className="button button--ghost" onClick={stopEditingMember} disabled={actionBusy}>
                            Cancel
                          </button>
                        ) : null}
                      </div>
                ...
                >
                  <ResponsiveTable
                    headers={["Photo", "Member", "Phone", "WhatsApp", "Telegram", "Status", "Joined", "Actions"]}
                    rows={filteredMembers.map((member) => [
                      <div key={`${member.id}-photo`}>
                        {member.profileImageDataUrl ? (
                          <img src={member.profileImageDataUrl} alt={member.fullName} style={{ width: 40, height: 40, borderRadius: 4, objectFit: "cover" }} />
                        ) : (
                          <div style={{ width: 40, height: 40, borderRadius: 4, backgroundColor: "#e5e5e5", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: "#999" }}>No photo</div>
                        )}
                      </div>,
                        ...
                        {invoice.status === "PAID" ? (
                          <>
                            ...
                            <button
                              ...
                              disabled={actionBusy}
                              onClick={() => handleSendReceiptPdf(invoice)}
                            >
                              Send receipt
                            </button>
                          </>
...
async function generateReceiptPdf(invoice: InvoiceResponse, member?: MemberResponse): Promise<Blob> {
  const element = document.createElement("div");
  element.innerHTML = buildReceiptDocument(invoice, member);

  return html2pdf()
    .set({
      margin: [receiptPdfMarginMm, receiptPdfMarginMm, receiptPdfMarginMm, receiptPdfMarginMm] as [number, number, number, number],
      filename: buildReceiptFileName(invoice),
...
function buildReceiptFileName(invoice: InvoiceResponse) {
  return `${invoice.invoiceNumber} - ${receiptFileLabel}.pdf`;
}
    ...
    <main class="sheet">
      <div class="logo-section">
        ...
        <div style="text-align: right;">
          <div class="brand" style="margin: 0;">${escapeHtml(gymBrand)}</div>
          <p style="margin: 4px 0 0 0; font-size: 12px; color: var(--muted);">Contact: info@gym.com | +91-XXXXXXXXXX</p>
        </div>
      </div>
      ...
      <table>
        ...
        Generated on ${escapeHtml(formatDate(new Date().toISOString()))} from ${escapeHtml(gymBrand)} Control Room
      </p>
    </main>
  </body>
</html>`;
...
function buildReceiptDocument(invoice: InvoiceResponse, member?: MemberResponse) {
    ...
    <main class="sheet">
      ...
      <div class="topline">
        ...
        <div>
          ...
          <p class="muted" style="margin-top: 12px;">On ${escapeHtml(formatDate(new Date().toISOString()))}</p>
        </div>
      </div>
      ...
      <div class="meta-grid">
        ...
        <section class="meta-card">
          ...
          <strong>${escapeHtml(formatCurrency(invoice.amountPaid))}</strong>
        </section>
        <section class="meta-card">
      ...
      <table>
        ...
        <tbody>
          ...
          ${invoice.taxAmount > 0 ? `<tr>
            <td>Tax</td>
            <td>${escapeHtml(formatCurrency(invoice.taxAmount))}</td>
          </tr>` : ""}
          ${invoice.discountAmount > 0 ? `<tr>
            <td>Discount</td>
            <td>-${escapeHtml(formatCurrency(invoice.discountAmount))}</td>
          </tr>` : ""}
          <tr>
            <td><strong>Total Paid</strong></td>
            <td><strong>${escapeHtml(formatCurrency(invoice.amountPaid))}</strong></td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="signature-section">
        <div class="signature-area">
          <img src="/images/signature.png" alt="Authorized Signature" />
      ...
      <p class="footer" style="text-align: center; margin-top: 24px; font-size: 12px;">
        Receipt generated on ${escapeHtml(formatDate(new Date().toISOString()))} from ${escapeHtml(gymBrand)} Control Room. Thank you for your payment!
      </p>
    </main>
  </body>
</html>`;
